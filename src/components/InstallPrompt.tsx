...
export default function InstallPrompt() {
  ...
  const panelTitle = variant === "ios" ? "Add to Home Screen" : "Install App";
  let summaryText = "Add to your home screen for quick access on fest day.";
  if (variant === "android") {
    summaryText =
      "Use the Install button here, or look for the install icon in the address bar.";
  } else if (variant === "ios") {
    summaryText = "Install for quick access on fest day.";
  }

  let expandedContent = (
    <p className="mt-2 text-xs text-zinc-300">
      Look for an <span className="font-bold text-white">install</span> option
      in your browser UI, usually an install icon in the address bar or an
      <span className="font-bold text-white"> Install App</span> option in the
      browser menu.
    </p>
  );
    ...
    expandedContent = (
      <ol className="mt-2 space-y-1 text-xs text-zinc-300 list-none">
        <li>
          ...
          Tap the{" "}
          <span className="inline-flex items-center gap-1 rounded-md border border-zinc-700 bg-zinc-800 px-2 py-0.5 font-bold text-white align-middle shadow-sm">
            Share
            {/* Common iOS share icon */}
            <svg
  ...
  const showInstallButton = variant === "android";
  const showHowToButton = variant !== "android";
    ...
    <div className="border-b border-zinc-800 bg-zinc-900">
      <div className="max-w-2xl mx-auto px-3 sm:px-4 py-3 flex items-start gap-2 sm:gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-zinc-400 mt-0.5">{summaryText}</p>
          {showHowToButton && iosExpanded ? expandedContent : null}
